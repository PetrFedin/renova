...
console.assert(
  inbox.includes("href: '/control?focus=warranty'") || inbox.includes('/quality-control'),
  'warranty→control/QC',
);
...
console.assert(chat.includes('notifyOfflineQueued') && chat.includes("notifyOfflineQueued('Счёт')"), 'invoice offline UI');

const ok = chat.includes('notifyOfflineQueued') && inbox.includes('/control?focus=warranty');
