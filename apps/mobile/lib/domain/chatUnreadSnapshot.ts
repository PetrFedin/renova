 ...
 * После успешного POST /read вызывающий store передаёт server total явно.
 */
export function patchThreadUnreadInSnapshot(
  ...
  const hasAuthoritativeTotal = typeof authoritativeTotal === 'number'
    && Number.isFinite(authoritativeTotal);
  const nextTotal = hasAuthoritativeTotal
