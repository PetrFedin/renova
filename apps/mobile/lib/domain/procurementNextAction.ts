...
/** Позиции, из которых текущая роль действительно может создать закупку. */
export function readyPickIds(
  picks: PickLike[],
...
export function procurementNextAction(
  ...
  const ready = readyPickIds(picks, purchases, role);
  const pendingApprove = picks.filter((p) => p.status === 'pending').length;
  if (pendingApprove > 0 && !ready.length) {
    return {
      id: 'approve_picks',
  ...
  if (missingExternal > 0) {
    return {
      id: 'confirm_supply',
      title: `${missingExternal} материал(ов) ждут подтверждения наличия`,
      subtab: 'picks',
      cta: 'К материалам',
    };
  }
  const open = purchases.find((p) => p.status !== 'delivered' && p.status !== 'cancelled' && p.status !== 'returned');
  if (open) {
    const step =
