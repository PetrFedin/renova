...
try {
  ...
  // Concurrent successful reads keep provenance bound to each exact result.
  ...
  // A stale durable value retains the ORIGINAL authoritative asOf across fallbacks.
  ...
  // Two stale keys coexist; there is no global "last outcome" overwrite.
  ...
  // Fresh-only req must never silently return durable cache.
  ...
  // Authoritative 4xx must not be hidden by stale data.
