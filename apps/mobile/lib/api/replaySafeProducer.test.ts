...
try {
  ...
  // Network ambiguity queues the EXACT first receipt command and identity.
  await resetQueue();
  ...
  // A deterministic 409 is authoritative and creates no hidden offline work.
  await resetQueue();
  ...
  // Malformed success is ambiguous: server may already have committed the write.
  await resetQueue();
  ...
  // Normal chat message is also server-idempotent; generated identity survives enqueue.
  await resetQueue();
  ...
  // A session-generation refusal is local authority, never offline ambiguity.
  await resetQueue();
