...
export const osApi = {
  ...
  exportReportPdf: async (userId: string, projectId: string, kind: 'daily' | 'weekly' | 'final') => { const { downloadReportPdf } = await import('@/lib/reports/reportPdf'); await downloadReportPdf(userId, projectId, kind); },
  ...
  activityFeed: (userId: string, projectId: string, kind?: string, workType?: string) => { const q = new URLSearchParams(); if (kind) q.set('kind', kind); if (workType) q.set('work_type', workType); const qs = q.toString(); return req<ActivityItem[]>(`/api/v1/projects/${projectId}/activity${qs ? `?${qs}` : ''}`, {}, userId); },
  exportActivityDossier: async (userId: string, projectId: string) => { const { downloadApiPath } = await import('@/lib/downloadFile'); await downloadApiPath(userId, `/api/v1/projects/${projectId}/activity-dossier.pdf`, 'dossier.pdf'); },
  exportFullDossier: async (userId: string, projectId: string) => { const { downloadApiPath } = await import('@/lib/downloadFile'); await downloadApiPath(userId, `/api/v1/projects/${projectId}/full-dossier.pdf`, 'full-dossier.pdf'); },
  exportProjectPdf: async (userId: string, projectId: string) => { const { downloadApiPath } = await import('@/lib/downloadFile'); await downloadApiPath(userId, `/api/v1/projects/${projectId}/export.pdf`, `project-${projectId.slice(0, 8)}.pdf`); },
  exportKpiWeeklyPdf: async (userId: string, projectId: string) => { const { downloadApiPath } = await import('@/lib/downloadFile'); await downloadApiPath(userId, `/api/v1/projects/${projectId}/kpi-weekly.pdf`, 'kpi-week.pdf'); },
  ...
  export1cPaymentsCsv: async (userId: string, projectId: string) => { const { exportProjectCsvFile } = await import('@/lib/exportProjectCsv'); await exportProjectCsvFile(userId, `/api/v1/projects/${projectId}/export/1c-payments.csv`, `renova-1c-${projectId.slice(0, 8)}.csv`); },
  export1cPaymentsXml: async (userId: string, projectId: string) => { const { exportProjectCsvFile } = await import('@/lib/exportProjectCsv'); await exportProjectCsvFile(userId, `/api/v1/projects/${projectId}/export/1c-payments.xml`, `renova-1c-${projectId.slice(0, 8)}.xml`); },
  export1cCommercemlXml: async (userId: string, projectId: string) => { const { exportProjectCsvFile } = await import('@/lib/exportProjectCsv'); await exportProjectCsvFile(userId, `/api/v1/projects/${projectId}/export/1c-commerceml.xml`, `renova-cml-${projectId.slice(0, 8)}.xml`); },
  exportBankRegisterCsv: async (userId: string, projectId: string) => { const { exportProjectCsvFile } = await import('@/lib/exportProjectCsv'); await exportProjectCsvFile(userId, `/api/v1/projects/${projectId}/export/bank-register.csv`, `renova-bank-${projectId.slice(0, 8)}.csv`); },
  previewWeeklyDigest: (userId: string, projectId: string) => req<{ ok: boolean; title: string; body: string; source: string; mode: string; kpi_path?: string; weekly?: { warranty_open?: number | null; warranty_overdue?: number | null; pending_acceptances?: number | null; open_issues_count?: number | null } }>(`/api/v1/projects/${projectId}/digest/weekly/preview`, {}, userId),
  pushWeeklyDigest: (userId: string, projectId: string) => req<{ ok: boolean; notified: number; source?: string; mode?: string; body?: string; document_id?: string; kpi_path?: string; ai_narrative?: boolean }>(`/api/v1/projects/${projectId}/digest/weekly`, { method: 'POST' }, userId),
  importBankStatement: (userId: string, projectId: string, csv_text: string, opts?: { create_expenses?: boolean }) => req<{ ok: boolean; parsed_rows: number; matched: number; unmatched_rows: number; expenses_created?: number; expenses_replayed?: number; match_token?: string | null; matches: { payment_id: string; payment_title: string; payment_status: string; payment_amount: number; score: number; date_match?: boolean | null; match_basis?: string | null }[] }>(`/api/v1/projects/${projectId}/import/bank-statement`, { method: 'POST', body: JSON.stringify({ csv_text, create_expenses: Boolean(opts?.create_expenses) }) }, userId),
  confirmBankStatementMatches: (userId: string, projectId: string, payment_ids: string[], match_token: string) => req<{ ok: boolean; confirmed: string[]; replayed: string[]; blocked: string[]; confirmed_count: number; replayed_count: number; blocked_count: number }>(`/api/v1/projects/${projectId}/import/bank-statement/confirm`, { method: 'POST', body: JSON.stringify({ payment_ids, match_token }) }, userId),
  createWarrantyClaim: async (userId: string, projectId: string, body: { title?: string; description?: string }) => {
