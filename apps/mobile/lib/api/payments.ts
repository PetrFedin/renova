...
async function uploadPaymentEvidenceBytes(userId: string, intent: PaymentEvidenceUploadIntent, uri: string): Promise<void> {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      reportError('payment.evidence.uploadResponse.parse', error, { status: response.status, evidenceId: intent.id });
...
export const paymentsApi = {
  ...
  getPaymentRequisites: (userId: string, projectId: string) => req<{ recipient_name?: string | null; payment_requisites?: string | null; phone?: string | null; has_bank_details: boolean }>(`/api/v1/projects/${projectId}/payment-requisites`, {}, userId),
  createPayment: async (userId: string, projectId: string, body: object) => {
    try { return await req<Payment>(`/api/v1/projects/${projectId}/payments`, { method: 'POST', body: JSON.stringify(body) }, userId); }
    catch (e) { if (!(e instanceof ApiError) || e.status >= 500) throw new Error(OFFLINE_PAYMENT_CREATE_BLOCKED); throw e; }
  },
  countPendingPayments: async (userId: string, projectId: string) => (await req<Payment[]>(`/api/v1/projects/${projectId}/payments`, {}, userId)).filter((p) => p.status === 'pending').length,
  checkoutYookassa: (userId: string, projectId: string, paymentId: string, body?: { portal_token?: string }) => req<{ demo?: boolean; provider?: string; payment_id?: string; yookassa_payment_id?: string | null; confirmation_url?: string | null; status?: string; message?: string }>(`/api/v1/projects/${projectId}/payments/${paymentId}/yookassa-checkout`, { method: 'POST', body: body ? JSON.stringify(body) : undefined }, userId),
  ...
  confirmPayment: async (userId: string, projectId: string, paymentId: string, opts?: { transfer_ack?: boolean }) => {
    ...
    try { return await req<Payment>(`/api/v1/projects/${projectId}/payments/${paymentId}/confirm`, { method: 'POST', body }, userId); }
    catch (e) { if (e instanceof ApiError) throw e; const { enqueue } = await import('@/lib/offlineQueue'); await enqueue({ path: `/api/v1/projects/${projectId}/payments/${paymentId}/confirm`, method: 'POST', body, userId }); throw new Error('offline_queued'); }
  },
