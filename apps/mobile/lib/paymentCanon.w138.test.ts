...
must(sheet.includes("type PaymentMutation = 'card' | 'confirm' | 'dispute' | 'resolveDispute' | null"), 'exact payment mutation state');
...
must(sheet.includes("loading={mutation === 'resolveDispute'}"), 'dispute resolution loading state');
