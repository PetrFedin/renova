/**
 * Панель «Ещё»: badge = только action-единицы; во «Входящих» — подписанные категории.
 */
...
import {
  resolveHeaderMoreBadgeFromCounters,
  resolveInboxMenuBadgesFromCounters,
...
export function OsSectionMenu({ role, iconOnly = true }: Props) {
  ...
  const { counters, taskBadge, chatUnread } = useInboxTasks(menuRole);
  ...
  const headerBadge = resolveHeaderMoreBadgeFromCounters(counters);
  const inboxBadges = resolveInboxMenuBadgesFromCounters(counters);
                ...
                >
                  ...
                  {link.id === 'inbox' ? (
                    <View style={s.inboxBadges}>
                      ...
                      <LabeledChip label="Согласования" count={inboxBadges.approvals} tone="warning" />
                      <LabeledChip label="Платежи" count={inboxBadges.payments} tone="warning" />
                      <LabeledChip label="Качество" count={inboxBadges.quality} tone="warning" />
                    </View>
                  ) : null}
                </Pressable>
              ))}
...
const s = StyleSheet.create({
  ...
  chipT: { color: RenovaTheme.colors.surface, fontSize: 9, fontWeight: '700' },
});
