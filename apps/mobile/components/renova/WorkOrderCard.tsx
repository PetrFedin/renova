...
export function WorkOrderCard({ wo, rooms, compact }: { wo: WorkOrder; rooms?: Room[]; compact?: boolean }) {
  ...
  return (
    <Pressable style={[s.row, compact && s.compact]} onPress={() => router.push({ pathname: '/work-order/[id]', params: { id: wo.id, returnTo: pathname } } as any)}>
      <View style={{ flex: 1 }}>
      ...
      {wo.chat_thread_id && !compact && (
        <Pressable onPress={() => router.push({ pathname: '/chat/[threadId]', params: { threadId: wo.chat_thread_id!, returnTo: pathname } } as any)}>
