...
@pytest.mark.asyncio
async def test_invite_phone_validates_role_and_logs_notification_failure(
    invite_store,
    monkeypatch,
    caplog,
):
    ...
    async def broken_notify(*_args, **_kwargs):
        raise RuntimeError("delivery unavailable")

    monkeypatch.setattr(notification_service, "notify", broken_notify)
    caplog.set_level(logging.ERROR, logger="app.services.team_service")
    async with session_factory() as db:
        result = await team_svc.invite_phone(db, team.id, first.phone, "viewer")

    assert result["ok"] is True
    assert "team membership notification failed" in caplog.text


def test_join_source_uses_atomic_conditional_update_and_has_no_silent_notification_pass():
    ...
    assert "logger.exception" in invite_block
    assert "except Exception:\n        pass" not in invite_block
