"""W62: warranty claim close is customer-only."""
import pytest
from httpx import ASGITransport, AsyncClient

from app.main import app
from app.api.deps import get_current_user
from app.db.session import get_db
from app.models.entities import Project, ProjectIssue, User, UserRole


@pytest.mark.asyncio
async def test_warranty_close_rejects_contractor(db):
    customer = User(id="c-w62", phone="+79990006201", role=UserRole.customer)
    contractor = User(id="k-w62", phone="+79990006202", role=UserRole.contractor)
    project = Project(
        id="p-w62",
        name="W62",
        renovation_type="cosmetic",
        customer_id=customer.id,
        contractor_id=contractor.id,
        budget_planned=1,
        budget_spent=0,
    )
    issue = ProjectIssue(
        id="i-w62",
        project_id=project.id,
        title="[Гарантия] Течь",
        description="кухня",
        status="open",
        severity="high",
    )
    db.add_all([customer, contractor, project, issue])
    await db.commit()

    async def _db():
        yield db

    async def _user():
        return contractor

    app.dependency_overrides[get_db] = _db
    app.dependency_overrides[get_current_user] = _user
    try:
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post(
                f"/api/v1/projects/{project.id}/warranty-claims/{issue.id}/close",
            )
            assert response.status_code == 403, response.text
            assert "warranty_close_customer_only" in response.text

            generic_response = await client.post(
                f"/api/v1/projects/{project.id}/issues/{issue.id}/close",
            )
            assert generic_response.status_code == 403, generic_response.text
            assert "warranty_close_customer_only" in generic_response.text
    finally:
        app.dependency_overrides.clear()
