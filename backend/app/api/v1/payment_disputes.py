...
class PaymentDisputeResolutionIn(BaseModel):
...
def _dispute_http_error(exc: ValueError) -> HTTPException:
    code = str(exc)
    if code in {
        "payment_dispute_reason_too_short",
    ...
    if code in {
        ...
        "payment_dispute_expense_state_conflict",
        "payment_dispute_unverified_expense_conflict",
    }:
        return HTTPException(409, detail={"code": code})
    if code.startswith("payment_dispute_transition_blocked:"):
        return HTTPException(409, detail={"code": code})
    if code.startswith("payment_dispute_resolution_blocked:"):
        return HTTPException(409, detail={"code": code})
...
@router.post(
    "/{project_id}/payments/{payment_id}/dispute/resolve",
    response_model=PaymentDisputeOut,
)
async def resolve_payment_dispute(
    project_id: str,
    payment_id: str,
    body: PaymentDisputeResolutionIn,
    user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    await require_project(db, project_id, user, write=True)
