...
class WarrantyClaimIn(BaseModel):
    ...
    # Required rather than best-effort: accepting keyless creates would leave the
    # timeout/offline replay duplication hole open for older clients.
...
def _idempotency_http_error() -> HTTPException:
    return HTTPException(
        ...
        detail={
            ...
            "message": "Этот гарантийный запрос уже использован с другими данными",
        },
    )
...
async def create_warranty_claim(
    ...
    """Create one canonical warranty Issue + Document across retries and replicas."""
    project = await require_project(db, project_id, user, write=True)
    try:
        result = await warranty_svc.create_or_replay_warranty_claim(
            db,
            project=project,
            user_id=user.id,
            title=body.title,
        ...
        raise HTTPException(
            409,
            detail={
                "code": "warranty_claim_idempotency_target_missing",
                "message": "Каноническое гарантийное обращение не найдено",
            },
        ) from exc
