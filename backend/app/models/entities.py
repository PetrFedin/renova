...
class Project(Base):
    ...
    budget_planned: Mapped[float] = mapped_column(MONEY, default=0)
    budget_spent: Mapped[float] = mapped_column(MONEY, default=0)
...
class Stage(Base):
    ...
    payment_amount: Mapped[float] = mapped_column(MONEY, default=0)
...
class MarginSnapshot(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=_uuid)
    project_id: Mapped[str] = mapped_column(String(36), ForeignKey("projects.id"), index=True)
    margin_estimated: Mapped[float] = mapped_column(MONEY, default=0)
...
class SelectionItem(Base):
    ...
    allowance: Mapped[float | None] = mapped_column(MONEY, nullable=True)
    price: Mapped[float] = mapped_column(MONEY, default=0)
    shop_url: Mapped[str | None] = mapped_column(String(512), nullable=True)
    shop_name: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class JobLead(Base):
    ...
    budget_hint: Mapped[float | None] = mapped_column(MONEY, nullable=True)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[JobLeadStatus] = mapped_column(Enum(JobLeadStatus), default=JobLeadStatus.open)
    pre_estimate: Mapped[float | None] = mapped_column(MONEY, nullable=True)
...
class JobLeadQuote(Base):
    ...
    pre_estimate: Mapped[float] = mapped_column(MONEY)
...
class BudgetLine(Base):
    ...
    planned_amount: Mapped[float] = mapped_column(MONEY, default=0)
