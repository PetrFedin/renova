...
async def verify_receipt(parsed: dict) -> dict:
    ...
    url = f"https://proverkacheka.nalog.ru:9999/v1/inns/*/kkts/{fn}/tickets/{fd}?fiscalSign={fp}&sum={parsed.get('amount', 0)}"
    try:
        async with httpx.AsyncClient(timeout=8) as c:
            r = await c.get(url)
            if r.status_code == 200:
                return {"verified": True, "message": "ФНС: чек подтверждён"}
    except Exception:
        pass
    if settings.environment != "production" and parsed.get("fn") and parsed.get("amount", 0) > 0:
        return {"verified": True, "message": "Dev: чек принят (ФНС offline)"}
    return {"verified": False, "message": "ФНС недоступна — сохранено без проверки"}

def verify_receipt_stub(parsed: dict) -> dict:
    ok = bool(parsed.get("fn") and parsed.get("fd"))
    return {"verified": ok, "message": "Stub OK" if ok else "Невалидный QR"}
