"""Cross-process runtime topology and worker heartbeat truth.

The API process must not infer worker health from process-local counters after the
runtime split. Worker instances publish short-lived heartbeats to the same Redis
already required by deployed Renova environments. Local heartbeat files exist
only for the container-level healthcheck of the worker process.
"""
...
def worker_instance_id() -> str:
    explicit = (os.getenv("RENOVA_WORKER_INSTANCE_ID") or "").strip()
    if explicit:
        return hashlib.sha256(explicit.encode("utf-8")).hexdigest()[:20]
