...
WORKFLOW_TEMPLATES: dict[str, dict] = {
    ...
    # Нейтральный шаблон для этапа, вид работ которого не определён.
    # Раньше такие этапы получали «Демонтаж» — из четырнадцати стандартных
    # этапов одиннадцать показывались как демонтаж, с чужими шагами
    # («Вывоз мусора») и чужим чек-листом («Коммуникации заглушены»).
    # Общий шаблон честнее подсказки наугад.
    "general": {
        "name": "Работы по этапу",
        "steps": ["Подготовка", "Выполнение", "Самопроверка", "Сдача"],
        "checklist": [
            "Материалы на объекте",
            "Рабочая зона подготовлена",
...
NAME_HINTS: list[tuple[str, str]] = [
    ...
    # Освещение — это электрика. Остальные стандартные названия («Стены»,
    # «Чистовая отделка», «Инженерные системы») покрывают сразу несколько
    # видов работ, и угадывать за них не нужно — им подходит общий шаблон.
    ("освещ", "electrical"),
]
...
def resolve_work_type(code: str | None = None, stage_name: str | None = None) -> str:
    ...
    for hint, wt in NAME_HINTS:
        if hint in n:
            return wt
    # Нераспознанный этап — общий шаблон, а не «Демонтаж». Ошибиться видом
    # работ хуже, чем честно не знать его: на карточке этапа «Подготовка»
    # стояли шаги демонтажа.
