...
import { API, pickPrimaryDemoProject, seedDemoCustomerSession, apiReachable, webReachable } from './helpers';
...
test.describe('P3-W14 Portal + documents UI', () => {
  ...
  test('documents hub opens after demo session', async ({ page, request }) => {
    ...
    const pid = pickPrimaryDemoProject(projects).id;
    await seedDemoCustomerSession(page, cust.id as string, pid);
