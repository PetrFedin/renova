#!/usr/bin/env node
/** Execute actual req -> calendar API -> AsyncStorage queue -> flush in Node. */
...
function harness({ storage = new Map(), network, failStorage = false } = {}) {
  ...
  return {
    api: load('@/lib/api/calendar').calendarApi,
...
const content = [
  'BEGIN:VCALENDAR',
  'BEGIN:VEVENT',
  'UID:transport-calendar-event',
  'SUMMARY:External event',
  'DTSTART;VALUE=DATE:20261005',
  'END:VEVENT',
  'END:VCALENDAR',
].join('\r\n');
const ok = (value = { ok: true, parsed: 1, updated_stages: 1, replayed: false }) =>
...

{
  ...
  const network = async (_url, options) => {
    ...
    return ok({ ok: true, parsed: 1, updated_stages: 1, replayed: true });
  };
  ...
  assert.equal(server.size, 1, 'restart replay must retain one logical calendar import intent');
...
for (const status of [429, 500, 502, 503]) {
  ...
  const parsed = JSON.parse(queue[0].body);
  assert.equal(parsed.content, content);
  assert.ok(parsed.client_request_id);
  scenarios += 1;
}
...
console.log(`Calendar import actual transport/queue contracts OK (${scenarios} scenarios; TypeScript ${ts.version}; external providers disabled)`);
