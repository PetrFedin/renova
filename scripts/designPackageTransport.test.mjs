#!/usr/bin/env node
/** Execute actual req -> design API -> AsyncStorage queue -> restart flush for #413. */
...
function harness({ storage = new Map(), network, failStorage = false } = {}) {
  ...
  return { api: load('@/lib/api/design').designApi, queue: load('@/lib/offlineQueue'), storage, requests };
}

const design = { title: 'Replay concept', file_key: 'designs/replay.pdf', notes: 'one intent' };
const ok = (id = 'canonical-design-package') => new Response(JSON.stringify({ id, version: 1, status: 'published' }), { status: 200 });
let scenarios = 0;

{
  ...
  const network = async (_url, options) => {
    ...
    assert.equal(body.title, design.title);
  ...
  await assert.rejects(first.api.createDesignPackage('actor-A', 'project-A', design), /offline_queued/);
...
for (const status of [400, 401, 403, 404, 409, 422]) {
  ...
  await assert.rejects(h.api.createDesignPackage('actor-A', 'project-A', design), (error) => error.status === status);
...
for (const status of [429, 500, 502, 503]) {
  ...
  await assert.rejects(h.api.createDesignPackage('actor-A', 'project-A', design), /offline_queued/);
...

{
  ...
  await assert.rejects(h.api.createDesignPackage('actor-A', 'project-A', design), /storage_unavailable/);
...
{
  const bodies = [];
  const h = harness({ network: async (_url, options) => { bodies.push(JSON.parse(options.body)); return ok(`design-${bodies.length}`); } });
  await h.api.createDesignPackage('actor-A', 'project-A', design);
  await h.api.createDesignPackage('actor-A', 'project-A', design);
  assert.notEqual(bodies[0].client_request_id, bodies[1].client_request_id, 'equal designs may be two deliberate versions');
  scenarios += 1;
}

{
  ...
  await h.api.createDesignPackage('actor-A', 'project-A', { ...design, client_request_id: 'design-explicit-001' });
  assert.equal(bodies[0].client_request_id, 'design-explicit-001');
  scenarios += 1;
}

console.log(`Design-package actual transport/queue contracts OK (${scenarios} scenarios; TypeScript ${ts.version})`);
