#!/usr/bin/env node
/** Execute actual req -> floor API -> AsyncStorage queue -> flush for waste-order create. */
...
const ok = (value = { id: 'waste-1', ...body, status: 'draft', total: 8750, replayed: false }) =>
  new Response(JSON.stringify(value), { status: 200 });
const invoke = h => h.api.createWasteOrder('actor-A', 'project-A', body);
let scenarios = 0;
...
// Lost response after server commit: restart must replay byte-identical original intent.
{
  ...
  const network = async (_url, options) => {
    ...
    assert.ok(typeof key === 'string' && key.startsWith('waste-'));
    assert.equal(parsed.room_id, 'room-A');
    assert.equal(parsed.volume_m3, 2.5);
    assert.equal(parsed.scheduled_date, '2026-09-20');
    assert.equal(parsed.price, 3500);
    ...
    return ok({ id: 'waste-1', ...body, status: 'draft', total: 8750, replayed: true });
  };
...

{
  ...
  assert.equal(result.id, 'waste-1');
...
console.log(`Waste-order actual transport/queue contracts OK (${scenarios} scenarios; TypeScript ${ts.version}; external providers disabled)`);
