#!/usr/bin/env node
/** Execute actual req -> floor API -> AsyncStorage queue -> flush for furniture create. */
...
const furnitureLayerSource = fs.readFileSync(path.join(mobile, 'components/renova/FurnitureLayer.tsx'), 'utf8');
assert.ok(!furnitureLayerSource.includes('enqueueOfflineCreate('), 'FurnitureLayer must not synthesize a second offline create payload');
...
const body = {
  ...
  x_pct: 30,
  y_pct: 60,
  notes: 'transport-proof',
};
const ok = (value = { id: 'furniture-1', ...body, replayed: false }) =>
  new Response(JSON.stringify(value), { status: 200 });
const invoke = h => h.api.createFurniture('actor-A', 'project-A', body);
let scenarios = 0;

// Lost response after server commit: restart must replay byte-identical original intent.
{
  ...
  const network = async (_url, options) => {
    ...
    assert.ok(typeof key === 'string' && key.startsWith('furniture-'));
    assert.equal(parsed.width_m, 2.1);
    assert.equal(parsed.depth_m, 0.9);
    ...
    return ok({ id: 'furniture-1', ...body, replayed: true });
  };
...
// Server may have committed but returned an unreadable 2xx body; treat as ambiguous replay-safe failure.
{
...

{
  ...
  assert.equal(result.id, 'furniture-1');
  assert.equal((await success.queue.getQueue()).length, 0);
  scenarios += 1;
}

// Two deliberate equal-visible user actions are different intents.
{
  const success = harness({ network: async () => ok() });
...
console.log(`Furniture actual transport/queue contracts OK (${scenarios} scenarios; TypeScript ${ts.version}; external providers disabled)`);
