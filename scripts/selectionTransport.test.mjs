#!/usr/bin/env node
/** Execute actual req -> selections API -> AsyncStorage queue -> restart flush for #415. */
...
function harness({ storage = new Map(), network, failStorage = false } = {}) {
  ...
  return { api: load('@/lib/api/selections').selectionsApi, queue: load('@/lib/offlineQueue'), storage, requests };
}

const selection = {
  title: 'Replay tile', category: 'tile', sku: 'RT-001', allowance: 4500,
  price: 4700, shop_name: 'Demo shop', notes: 'one intent',
};
const ok = (id = 'canonical-selection') => new Response(JSON.stringify({ id, project_id: 'project-A', ...selection, status: 'draft' }), { status: 200 });
let scenarios = 0;

{
  ...
  const network = async (_url, options) => {
    ...
    assert.equal(body.title, selection.title);
  ...
  await assert.rejects(first.api.createSelection('actor-A', 'project-A', selection), /offline_queued/);
...
for (const status of [400, 401, 403, 404, 409, 422]) {
  ...
  await assert.rejects(h.api.createSelection('actor-A', 'project-A', selection), (error) => error.status === status);
...
for (const status of [429, 500, 502, 503]) {
  ...
  await assert.rejects(h.api.createSelection('actor-A', 'project-A', selection), /offline_queued/);
...

{
  ...
  await assert.rejects(h.api.createSelection('actor-A', 'project-A', selection), /storage_unavailable/);
...
{
  const bodies = [];
  const h = harness({ network: async (_url, options) => { bodies.push(JSON.parse(options.body)); return ok(`selection-${bodies.length}`); } });
  await h.api.createSelection('actor-A', 'project-A', selection);
  await h.api.createSelection('actor-A', 'project-A', selection);
  assert.notEqual(bodies[0].client_request_id, bodies[1].client_request_id, 'equal selections may be separate user intents');
  scenarios += 1;
}

{
  ...
  await h.api.createSelection('actor-A', 'project-A', { ...selection, client_request_id: 'selection-explicit-001' });
  assert.equal(bodies[0].client_request_id, 'selection-explicit-001');
  scenarios += 1;
}

console.log(`Selection actual transport/queue contracts OK (${scenarios} scenarios; TypeScript ${ts.version})`);
