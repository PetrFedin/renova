...
If red-team, legacy cleanup, E2E review or incident analysis discovers a new P0/P1 launch-relevant gap, the same work wave must either fix it or create an explicit issue **and add that issue to the machine-readable readiness blocker set**. A critical finding must never disappear merely because the historical PR or document that exposed it is closed.
