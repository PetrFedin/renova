...
- The route performs normal project-write and `field_write` authorization before entering the mutation service.
- After any wait on the project row lock, the service reloads current project/user truth and re-runs project-write plus `field_write` authorization before replay resolution or mutation. Revoked access therefore wins over a stale pre-lock authorization decision.

## Concurrency boundary

Before a candidate `ProjectIssue` or outbox row is materialized, the service acquires a PostgreSQL row lock on the owning `Project`, reloads current authority state, and then re-checks the request ledger while holding that lock. Concurrent issue creates for the same project therefore serialize at the boundary before mutation:

1. the first transaction acquires the project lock, revalidates authority, checks the ledger, persists the issue/effects/ledger, and commits;
2. the second transaction then acquires the lock, reloads authority state, and performs a fresh ledger lookup;
...
5. if project access or `field_write` authority was revoked while the request waited, the request fails closed before replay/mutation.
...
1. `ProjectIssue`.
2. `DomainOutbox` activity event (`IssueCreated`) with the same business fields as the previous synchronous activity call.
